...
## Deploy to gh-pages (using GitHub Actions when on a nextjs project)?
- edit source in `portfolio` branch.

- make sure next.config.js has an option for `output: "export"` and  `distDir: "build"` exist.

- local build with `npm run build` to get the build folder.

- for deployment, push changes from `origin portfolio` to `origin release` branch.

- setup (once) at Github Project Settings => Pages => GitHub Actions

## How to use (Deploy with gh-pages branch on a create-react-app project)? 
...
## Git tag version the complicate steps:
...
So you set up your GitHub Page for yourself or project, but what if you want to show off some of your other projects you are working on. You can go the poor man's route, and simply just copy everything from another repository into a folder in your _eddienguyen.github.io_ repository, commit and push the changes to GitHub, and you'll be able to navigate to _http://eddienguyen.github.io/some-project_.
...
2. Add a submodule
  ...
  `git submodule add https://github.com/eddienguyen/other-repository`
...
3. Add/commit your new files
  ...
  `git add other-repository/`
  
  `git commit -m "Added other-repository submodule"`

4. Push your changes
  
  `git push origin master`

5. navigate to _http://eddienguyen.github.io/other-repository_
...
5. Add/commit your updated files
  ...
  `git commit -m "Updated other-repository submodule"`
...
## Below is the instruction built-in with `create-next-app` package.
